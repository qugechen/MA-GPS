...
from MAGPS.data import Batch, ReplayBuffer, to_numpy, to_torch_as
from MAGPS.utils import MultipleLRSchedulers


class MARL_BasePolicy(ABC, nn.Module):
    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    def update(self, sample_size: int, buffer: Optional[ReplayBuffer], behavior_loss_weight: float = 0.0,
        ...
        result = self.learn(batch, behavior_loss_weight, **kwargs)
    ...
    def compute_episodic_return(
        ...
        """
        # rew = batch.rew
        cost = batch.info.individual_cost
        bsz = len(indices)
        if v_s_ is None:
            assert np.isclose(gae_lambda, 1.0)
            v_s_ = np.zeros_like(cost)
        else:
            v_s_ = to_numpy(v_s_.reshape(bsz, num_players)) # double check!
            if v_s is None:
                v_s = np.zeros_like(v_s_)
                for i in range(num_players):
                    v_s_[:,i] = v_s_[:,i] * MARL_BasePolicy.value_mask(buffer, indices)
                    v_s[:,i] = np.roll(v_s_[:,i], 1)
            else:
                for i in range(num_players):
                    v_s_[:,i] = v_s_[:,i] * MARL_BasePolicy.value_mask(buffer, indices)
                    v_s[:,i] = to_numpy(v_s[:,i].flatten())
        end_flag = np.logical_or(batch.terminated, batch.truncated)
        end_flag[np.isin(indices, buffer.unfinished_index())] = True
        advantage = np.zeros((bsz, num_players))
        returns = np.zeros((bsz, num_players))
        for i in range(num_players):
            advantage[:,i] = _gae_return(v_s[:,i], v_s_[:,i], cost[:,i], end_flag, gamma, gae_lambda)
            returns[:,i] = advantage[:,i] + v_s[:,i]
    ...
    def compute_nstep_return(
        ...
        rew = buffer.rew
        costs = buffer.info.individual_cost
        bsz = len(indice)
        indices = [indice]
        ...
        target_q = to_numpy(target_q_torch) # TODO: how is this reshaped?
        for i in range(num_players):
            target_q[:, i] = target_q[:, i] * MARL_BasePolicy.value_mask(buffer, terminal)
        
        # target_q = target_q * MARL_BasePolicy.value_mask(buffer, terminal).reshape(-1, 1)
        end_flag = buffer.done.copy()
        end_flag[buffer.unfinished_index()] = True
        target_q = _nstep_return(costs, end_flag, target_q, indices, gamma, n_step, num_players)
    ...
    def _compile(self) -> None:
        ...
        f64_2d = np.array([[0,1], [0,1]], dtype=np.float64)
        f32 = np.array([0, 1], dtype=np.float32)
        f32_2d = np.array([[0,1], [0,1]], dtype=np.float32)
        b = np.array([False, True], dtype=np.bool_)
        i64 = np.array([[0, 1]], dtype=np.int64)
        _gae_return(f64, f64, f64, b, 0.1, 0.1)
        _gae_return(f32, f32, f64, b, 0.1, 0.1)
        _nstep_return(f64_2d, b, f32_2d.reshape(-1, 2), i64, 0.1, 1, 2)
...
def _nstep_return(
    ...
    for i in range(num_players):
        ...
        now = indices[0]
        returns[end_flag[now] > 0] = 0# NOTE: previous is 0
        returns = rew[now, i]
        target_q[:, i] = target_q[:, i] * gamma + returns
    # change target_q to 2d array
    # target_q = target_q.reshape(-1, 1)
    # returns = np.zeros((bsz, 1))
    ...
    return target_q #target_q.reshape(target_shape)
